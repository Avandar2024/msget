...
Run `./msget -h` for all options. The downloader automatically selects 2-8 connections based on available CPUs. Multiple files are downloaded concurrently, and files of at least 8 MiB use a dynamic queue of 8-64 MiB ranges served by up to four connections. The range size adapts to the file size and available concurrency, keeping enough queued work for faster workers without creating excessive requests for very large files. Failed transfers are retried five times and server-provided SHA-256 hashes are verified. Connections time out only when connecting or after 60 seconds without data; large files have no total time limit.
