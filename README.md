...
常用参数可用 `./msget -h` 查看。默认最多使用 8 条下载连接；多个文件会并发下载，8 MiB 以上的大文件默认拆成 4 个分片并行下载（可用 `-workers` 和 `-parts` 调整）。失败重试 5 次，并校验服务端提供的 SHA-256。连接或连续 60 秒收不到数据才会超时，不限制大文件的总下载时长。未完成的文件和分片状态保存为 `.part` / `.part.meta`，再次执行相同命令会继续下载。若服务器不支持 HTTP Range，会自动退回单连接下载。
